...
import { getTopRateMovies, IGetPopularMoviesResult } from "../../api";
...
function PopularMovie() {
  ...
  const bigMovieMatch = useMatch("movies/3/:movieId");
  const movieId = bigMovieMatch?.params.movieId;
  const onBoxClicked = (movieId: number) => {
    history(`/movies/3/${movieId}`);
  };
                ...
                .map((movie) => (
                  ...
                  >
                    <Info variants={infoVariants}>
                      <h4>{movie.title}</h4>
                      <OpenDate>{movie.release_date}</OpenDate>
